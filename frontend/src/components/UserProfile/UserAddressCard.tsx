...
export default function UserAddressCard({ user, onUserUpdate }: UserAddressCardProps) {
  const { isOpen, openModal, closeModal } = useModal();
  const [formData, setFormData] = useState({
    country: user.profile?.country || '',
    city: user.profile?.city || '',
    address: user.profile?.address || '',
    birth_date: user.profile?.birth_date ? new Date(user.profile.birth_date).toISOString().split('T')[0] : ''
  });
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">
                  {user.profile?.birth_date 
                    ? new Date(user.profile.birth_date).toLocaleDateString('ko-KR')
                    : '정보 없음'
                  }
                </p>
              </div>
