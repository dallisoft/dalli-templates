...
export default function UserDropdown() {
  ...
  const loadUserProfile = async () => {
    ...
    try {
      ...
      const userData = await userProfileService.getUserWithProfile(user.id);
      setUserWithProfile(userData);
    } catch (error) {
      console.error('Error loading user profile:', error);
      // If profile doesn't exist, just use basic user data
      setUserWithProfile({
        ...user,
        profile: undefined
      });
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11">
          {isLoadingProfile ? (
            ...
            <img 
              src={userWithProfile?.profile?.avatar_url || "/images/user/owner.jpg"} 
